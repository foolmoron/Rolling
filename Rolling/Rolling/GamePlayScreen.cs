...
namespace Rolling
{
    class GamePlayScreen : GameScreen
    {
...
#else
            sky.Draw(camera.View, camera.Projection);
            DrawModelBasic(terrainModel, Matrix.Identity, rockTexture);
            DrawModelBasic(ballModel, ball.World, currentBallProperties.Texture);
            DrawModelBasic(arrowModel, Matrix.CreateScale(.5f) * ArrowRotation * Matrix.CreateTranslation(ball.position) * Matrix.CreateTranslation(0, 50f, 0), null);
            DrawModelBasic(exitModel, Matrix.CreateScale(0.1f) * Matrix.CreateTranslation(ExitPosition), null);
            DrawModelBasic(pitModel, Matrix.CreateTranslation(new Vector3(0, pitHeight, 0)), terrainTexture.Equals("Textures\\2") ? pitTextureIce : pitTextureLava);

            foreach (Coin x in coinList)
            {
                DrawModelBasic(x.getModel(), Matrix.CreateScale(.2f) * x.getWorld(), coinTexture);
            }

            foreach (teleporter x in teleporterList)
            {
                DrawModelBasic(x.getModel(), Matrix.CreateScale(2f) * x.getWorld(), teleporterTexture);
            }
#endif
            //particleEngine.Draw(spriteBatch);

            spriteBatch.Begin();
            spriteBatch.DrawString(spriteFont, "Time Left: " + (LevelTime / 1000).ToString("#0.0"), new Vector2(device.Viewport.Bounds.Width / 2 - 50, 25), Color.White);
            if (debugging)
            {
        ...
        private void DrawModelBasic(Model model, Matrix world, Texture2D texture)
        {
